...
#include <vnet/devices/odp/odp_packet.h>
			   ...
			   vlib_frame_t * frame, odp_packet_if_t *oif)
{
  ...
  n_free_bufs = vec_len (om->rx_buffers[cpu_index]);
  if (PREDICT_FALSE (n_free_bufs < VLIB_FRAME_SIZE))
    {
      vec_validate (om->rx_buffers[cpu_index],
		    VLIB_FRAME_SIZE + n_free_bufs - 1);
      n_free_bufs +=
	vlib_buffer_alloc (vm, &om->rx_buffers[cpu_index][n_free_bufs],
			   VLIB_FRAME_SIZE);
      _vec_len (om->rx_buffers[cpu_index]) = n_free_bufs;
            ...
            while (data_len && n_free_bufs)
              {
                   ...
                   vec_len (om->rx_buffers[cpu_index]) - 1;
                   prev_bi0 = bi0;
                   bi0 = om->rx_buffers[cpu_index][last_empty_buffer];
                   b0 = vlib_get_buffer (vm, bi0);
                   _vec_len (om->rx_buffers[cpu_index]) = last_empty_buffer;
                   n_free_bufs--;
                   /* copy data */
                   u32 bytes_to_copy =
  ...
  vlib_increment_combined_counter(vnet_get_main ()->interface_main.combined_sw_if_counters
     ...
     os_get_cpu_number (), oif->hw_if_index, n_rx_packets, n_rx_bytes);
  ...
  for (i = 0; i < vec_len (om->interfaces); i++)
    {
      ...
      if (oif->is_admin_up &&
         ...
         (cpu_index - om->input_cpu_first_index))
        {
